...
DEPLOY_OUTPUT=$(npx hardhat run scripts/deploy.js --network swisstronik)
echo "Contract deployed."
echo "$DEPLOY_OUTPUT" >> transaksi.txt
...
async function main() {
  ...
  echo "Transaction Receipt: ", setMessageTx >> ../transaksi.txt
}
...
SETMESSAGE_OUTPUT=$(npx hardhat run scripts/setMessage.js --network swisstronik)
echo "Message set."
echo "$SETMESSAGE_OUTPUT" >> transaksi.txt
...
async function main() {
  ...
  echo "Decoded response:", contract.interface.decodeFunctionResult(functionName, responseMessage)[0] >> ../transaksi.txt
}
...
GETMESSAGE_OUTPUT=$(npx hardhat run scripts/getMessage.js --network swisstronik)
echo "Message retrieved."
echo "$GETMESSAGE_OUTPUT" >> transaksi.txt

echo "Done! Subscribe: https://t.me/HappyCuanAirdrop"

# Menambahkan username dan wallet name ke dalam transaksi.txt
echo "Username: $USERNAME, Wallet: $WALLET_NAME" >> transaksi.txt
